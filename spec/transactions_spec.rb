...
describe 'Transactions' do
  describe '.sender_from_row' do
    ...
    it 'gets sender from row' do
      @trans.sender_from_row.should == @account
    end

    it 'fails to get sender from row' do
      @row.stub(:sender) { nil }
      @trans.sender_from_row.should be nil
      @trans.errors.size.should eq 1
      @trans.errors.first.should eq '12: Account 000000001 not found'
    end
  end
  ...
  context 'AccountTransfer' do
    ...
    describe '.proc_transaction, DEPOT_ACTIVITY_ID is blank' do
      ...
      it 'returns error if sender not found' do
        @row[Row::HEADERS[:activity]] = 12
        prepare_instance_vars(@row) do
          stub_const('Account', Class.new)
    ...
    describe '.proc_transaction, DEPOT_ACTIVITY_ID is not blank' do
      it 'finds and validates account_transfer' do
        prepare_instance_vars(@row) do |row|
          @account_transfer.stub(id: 1, state: 'pending', 'subject=': nil,
                                 valid?: true, complete_transfer!: true)
        ...
        @trans.proc_transaction.should be true
        @trans.errors.size.should eq 0
      end

      it 'fails to find account transfer' do
        @row[Row::HEADERS[:depot_activity_id]] = 12_345
        prepare_instance_vars(@row)
        @account.stub_chain :credit_account_transfers, find_by_id: nil
        @trans.proc_transaction.should be nil
        @trans.errors.size.should eq 1
        @trans.errors.first.should eq ': AccountTransfer not found'
      end
      ...
      it 'finds account transfer, but is not in pending state' do
        ...
        @trans.errors.size.should eq 1
        @trans.errors.first
              .should eq(': AccountTransfer state expected \'pending\' but was \'initialized\'')
      end
    end
  end
  ...
  context 'BankTransfer' do
    describe '.proc_transaction' do
      ...
      it 'fails to add bank transfer' do
        ...
        @trans.proc_transaction.should be nil
        @trans.errors.first.should eq ': Account 000000001 not found'
        @trans.errors.size.should eq 1
      end
    end
  end
  ...
  context 'Lastschrift' do
    describe '.proc_transaction' do
      ...
      it 'adds dta row' do
        ...
        @dtaus.should_receive(:add_buchung)
              .with('0101881952', '30020900', 'Max Mustermann', 10, 'Subject').once
        @trans.dtaus = @dtaus
      ...
      it 'fails to adds dta row' do
        ...
        @trans.dtaus = @dtaus
        @trans.proc_transaction.should be nil
        @trans.errors.first.should eq '1: BLZ/Konto not valid, csv fiile not written'
        @trans.errors.size.should eq 1
      end
    end
  end
